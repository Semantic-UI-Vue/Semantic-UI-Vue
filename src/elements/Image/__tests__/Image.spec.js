...
describe('Image', () => {
  testClassFromProps(ImageWithRequired, [
    'disabled', 'hidden',
  ]);
  ...
  it('should create a wrapped Image', () => {
    const image = shallowMount(ImageWithRequired, { propsData: { wrapped: true } });
  ...
  it('should display user image', () => {
    ...
    const wrappedImage = shallowMount(ImageWithRequired, { propsData: { src, wrapped: true } });
    expect(wrappedImage.find('img').element.getAttribute('src')).toEqual(src);
  });

  it('should work with spaced prop', () => {
    const image = shallowMount(ImageWithRequired, { propsData: { spaced: true } });
    expect(image.classes()).toContain('spaced');
    expect(image.classes()).not.toContain('right');
    expect(image.classes()).not.toContain('left');

    const imageWithoutSpace = shallowMount(ImageWithRequired, { propsData: { spaced: false } });
    ...
    const imageLeftSpace = shallowMount(ImageWithRequired, { propsData: { spaced: 'left' } });
    ...
    const imageRightSpace = shallowMount(ImageWithRequired, { propsData: { spaced: 'right' } });
  ...
  it('should create image with size prop', () => {
    const image = shallowMount(ImageWithRequired, { propsData: { size: 'mini' } });
    expect(image.classes()).toContain('mini');
  });

  it('should create image with shape prop', () => {
    const image = shallowMount(ImageWithRequired, { propsData: { shape: 'rounded' } });
